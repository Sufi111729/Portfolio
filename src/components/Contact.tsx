...
const Contact = () => {
  ...
  // ✅ Initialize EmailJS
  useEffect(() => {
    emailjs.init('uSqxCCC9avDJlSHWk'); // 🔑 Your Public Key
  }, []);
  ...
  return (
    <section id="contact" className="py-16 md:py-24 bg-dark-light/50">
      <div className="container mx-auto px-4 md:px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="max-w-3xl mx-auto text-center mb-16">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <div className="glass-card rounded-lg p-6 md:p-8">
              ...
              <form onSubmit={handleSubmit}>
                ...
                {submitMessage && (
                  <p className={`mt-4 text-center ${submitMessage.includes('Oops') || submitMessage.includes('❌') ? 'text-red-500' : 'text-green-500'}`}>
                    {submitMessage}
                  </p>
                )}
              </form>
